...
forge script script/Hyperlane/DeployCounter.s.sol:DeployCounterScript --rpc-url $ALCHEMY_RPC_GOERLI --broadcast --verify

# Send the source transaction
echo "Sending the source transaction on network: $HYPERLANE_DESTINATION_DOMAIN"
forge script script/Hyperlane/CounterSourceTx.s.sol:CounterSourceTxScript --rpc-url $ALCHEMY_RPC_MUMBAI --broadcast